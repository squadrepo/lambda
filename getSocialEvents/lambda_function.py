...
#
# Lambda fn fetch social events from the past week
#
###########################################################
...
def lambda_handler(event, context):
    ...
    if 'fromTime' in data:
        try:
            timeEnd = int(data['fromTime']) * 1000
            timeFrom = timeEnd - NUM_SECS_IN_WK
        except:
            return {
                'statusCode': 404,
                'body': json.dumps({
        ...
        if pKey == 'univAssoc':
            response = table.query(
                KeyConditionExpression=Key(pKey).eq(pKeyVal)&Key('createTimestamp').between(timeFrom,timeEnd)
                )
        else:
            response = table.query(
                IndexName=pKey,
